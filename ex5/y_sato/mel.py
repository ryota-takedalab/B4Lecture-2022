...
if __name__ == "__main__":
    ...
    tmp = signal * win_fc
    t_original_spec = np.abs(np.fft.fft(tmp))[: window // 2]
    t_log_spec = 20 * np.log10(t_original_spec)
    mspec = t_original_spec @ filterbank.T
    nceps = 12
    mfcc = dct(20 * np.log10(mspec))[:nceps]
    """
    ...
    xline = np.linspace(0, sr // 2, window // 2)
    ax2.plot(xline, 20 * np.log10(t_original_spec), label="Original Spectrum")
    ax2.plot(fcenters, 20 * np.log10(mspec), "o-", label="Mel Spectrum")
    plt.show()
    """
